...
import { r as requiredArgs, t as toDate, c as toInteger, _ as _typeof, b as getDefaultOptions, l as locale, g as getTimezoneOffsetInMilliseconds, m as millisecondsInHour, d as millisecondsInMinute, e as millisecondsInSecond, a as assign, p as parseISO } from '../chunks/index_Ck9nm2cs.mjs';
...
async function getStories$1() {
  try {
    const url = `https://api.rss2json.com/v1/api.json?rss_url=${MEDIUM_RSS}`;
    const res = await fetch(url);
    if (!res.ok) {
      console.warn(`rss2json API returned ${res.status}`);
      return [];
    }
    const data = await res.json();
    if (!data || !data.items) {
      console.warn('rss2json returned unexpected data');
      return [];
    }
    return data.items.map((post) => {
      const date = parse(post.pubDate, 'yyyy-MM-dd HH:mm:ss', 0);
      const readableDate = format(date, 'd.M, yy');
      return {
        url: post.link,
        title: post.title,
        date,
        readableDate,
        slug: post.guid.split('/')[4]
      };
    });
  } catch (e) {
    console.warn('Failed to fetch Medium stories:', e.message);
    return [];
  }
}
...
async function getStories() {
  try {
    const url = `https://dev.to/api/articles?username=${DEVTO_USERNAME}`;
    const res = await fetch(url);
    if (!res.ok) {
    ...
    console.warn('Failed to fetch dev.to stories:', e.message);
    return [];
  }
}
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const mediumPosts = await getStories$1();
  const devtoPosts = await getStories();
  const posts = [...mediumPosts, ...devtoPosts].sort((a, b) => b.date - a.date);
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": "Notes" }, { "default": async ($$result2) => renderTemplate`  ${maybeRenderHead()}<div class="c-small"> <p class="text-2xl text-highlight">Notes to myself</p> <p>
My Medium and dev.to articles are automatically added here :)
</p> <div class="mt-20"> ${posts.length ? posts.map((post) => renderTemplate`<article class="mb-10"> <a${addAttribute(post.url, "href")} target="_blank" rel="noopener noreferrer" class="text-lg leading-6 font-bold text-highlight hover:underline"> ${post.title} </a> <p> <time>${post.readableDate}</time> </p> </article>`) : renderTemplate`<p>No notes written yet. Interesting...</p>`} </div> </div>  `, "footer": async ($$result2) => renderTemplate`${renderComponent($$result2, "Fragment", Fragment, { "slot": "footer" }, { "default": async ($$result3) => renderTemplate` ${renderComponent($$result3, "Footer", $$Footer, {})} ` })}`, "header": async ($$result2) => renderTemplate`${renderComponent($$result2, "Fragment", Fragment, { "slot": "header" }, { "default": async ($$result3) => renderTemplate` ${renderComponent($$result3, "Header", $$Header, { "active": "/notes" })} ` })}` })}`;
}, "/home/calgan/homepage-rewrite/src/pages/notes/index.astro", void 0);

const $$file = "/home/calgan/homepage-rewrite/src/pages/notes/index.astro";
const $$url = "/notes";
